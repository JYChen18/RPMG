...
def eval_cates(rslt_txt_file, rc_tbl, cates=['aeroplane', 'boat', 'car'], theta_levels_str='pi/6'):
    ...
    for cate in cates:
        ...
        # exit()
        MedError, Acc_at_ts, geo_dists = eval_one(_pr_quats, _gt_quats,
                                                  theta_levels=theta_values)  # eval string express: to convert to float number.
    ...
    rslt_lines.append(txtTbl.format('MEAN', np.mean(list_MedError),
                                    *['%10.3f' % (np.mean(theta_level2list_Acc_at_t[theta_level]) * 100) for theta_level
                                      in theta_strs]))
    summary_str = '\n'.join(rslt_lines) + '\n'
    # print summary_str
    return summary_str
