...
class npy_table:
    ...
    def __init__(self, recarr, primary_key=None):
        self.dtype = recarr.dtype
        self.shape = recarr.shape
        self.primary_key = primary_key
        if self.primary_key is None:
            self.primary_key = self.dtype.names[
                0]  # default primary_key for an np.array is the first field of the dtype.
        # ---------------------------------------
        ...
        assert len(self.keys) == len(set(self.keys)), '[Error] keys of primary_key are not unique: %s != %s ' % (
        len(self.keys), len(set(self.keys)))
        #
...
def reorder_dtype(old_dtype, primary_key):
    ...
    new_dtype = [(fname, old_dtype.fields[fname][0]) for fname in
                 new_field_names]  # dtype.fields return (dtype, offset[, title])
...
class npy_db:
    ...
    def __str__(self):
        ...
        for name, tb in self.name2table.items():
            lines.append('   [Table] %-15s %-15s %s' % (
            '"%s"' % name, 'shape=%s' % str(tb.recs.shape), str(tb.dtype)[:50] + '  ...'))
        return '\n'.join(lines) + '\n'
    ...
    def dump(self, outfile):  # pkl  or hdf5  # method='pickle'  or hdf5 h5py
        ...
        for name, table in self.name2table.items():
            assert isinstance(table.recs, np.ndarray) or isinstance(table.recs, np.recarray), '[Exception] %s' % (
                type(table.recs))
            ...
            assert table.primary_key == table.recs.dtype.names[
                0], '[Exception] table "%s" has non-first field primary_key "%s"' % (name, table.primary_key)
    ...
    def load(self, infile):
        ...
        if infile.endswith('pkl'):
            name2recarr = pickle.load(open(infile, 'rb'), encoding='latin1')
            for name, recarr in name2recarr.items():
                self.name2table[name] = npy_table(recarr)
        elif infile.endswith('pkl.gz'):
            name2recarr = pickle.load(gzip.open(infile, 'rb'), encoding='latin1')
            for name, recarr in name2recarr.items():
                self.name2table[name] = npy_table(recarr)
...
def test_table_db2():
    ...
    db = npy_db()
    db.load('test_db.pkl', encoding='latin1')
