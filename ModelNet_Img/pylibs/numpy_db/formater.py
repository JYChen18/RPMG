...
def fmt_listdef(lines_str, indent=8):
    ...
    def code_line(line):
        ...
        if _ind > 0:  # have shape definition.
            ...
            data_type = str2type.get(data_type,
                                     data_type)  # data_type if data_type not in str2type.keys() else str2type[data_type]
            _code_line = " " * indent + "(%-20s , %-20s, %-12s ),  %s" % (
            "'%s'" % field_name, data_type, data_shape, comm_str)
        else:
            data_type = _rest
            data_type = str2type.get(data_type, data_type)
            _code_line = " " * indent + "(%-20s , %-20s  %s ),  %s" % (
            "'%s'" % field_name, data_type, ' ' * 12, comm_str)
